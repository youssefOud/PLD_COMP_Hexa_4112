...
string IFInstruction::buildIR(CFG *cfg)
{
    ...
    BasicBlock *thenBB = new BasicBlock(cfg, ".trueCode_"+to_string(cfg->getCmptFct())+"_"+to_string(cfg->getNbIf()));
    cfg->add_bb(thenBB);

    BasicBlock *elseBB = new BasicBlock(cfg, ".falseCode_"+to_string(cfg->getCmptFct())+"_"+to_string(cfg->getNbIf()));
    cfg->add_bb(elseBB);


    BasicBlock *afterIfBB = new BasicBlock(cfg, ".afterIfBB_"+to_string(cfg->getCmptFct())+"_"+to_string(cfg->getNbIf()));
